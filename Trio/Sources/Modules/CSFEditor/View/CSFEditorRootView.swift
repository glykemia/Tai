    ...
    struct RootView: BaseView {
        ...
        var body: some View {
            ScrollViewReader { proxy in
                    ...
                    ScrollView {
                        LazyVStack {
                            VStack(alignment: .leading, spacing: 0) {
                                ...
                                // Suggestion button when profile is empty
                                if state.items.isEmpty {
                                    suggestDefaultButton
                                        .padding(.horizontal)
                                        .padding(.top)
                                }
                ...
                .navigationTitle("Carb Sensitivities")
        ...
        // Suggest default CSF button
        private var suggestDefaultButton: some View {
            Button {
                state.suggestDefaultCSF()
                state.therapyItems = state.getTherapyItems()
            } label: {
                Text("Suggest CSF from ISF & CR at 1pm")
        ...
        // Chart for visualizing CSF profile
        private var csfChart: some View {
            Chart {
                ForEach(Array(state.items.enumerated()), id: \.element.id) { index, item in
                    ...
                    LineMark(x: .value("End Date", startDate), y: .value("CSF", displayValue))
                        .lineStyle(.init(lineWidth: 1)).foregroundStyle(Color.orange)

                    LineMark(x: .value("Start Date", endDate), y: .value("CSF", displayValue))
                        .lineStyle(.init(lineWidth: 1)).foregroundStyle(Color.orange)
                }
            }
